    ...
    for row in range(class_id_image.shape[0]):
        for col in range(class_id_image.shape[1]):
            try:
                ...
                # print("class_id_to_rgb_map",class_id_image[row, col], class_id_to_rgb_map[int(class_id_image[row, col])].name)
                color_names.append(class_id_to_rgb_map[int(class_id_image[row, col])].name)

            except KeyError as key_error:
                print("Warning: could not resolve classid %s" % key_error)
    return colored_image,color_names
...
def color_class_image(class_image, model_name):
    ...
    if 'cityscapes' in model_name:
        ...
        colored_image,color_names = class_image_to_image(class_image, ade20k_id2label)
    else:
